        ...
        div(class="justify-between items-center md:hidden hidden lg:flex", v-if="user_connected && Object.keys(user_connected).length > 0")
            ...
            div
                div(class="dropdown inline-block relative")
                    ...
                    ul( class="dropdown-menu absolute hidden text-gray-700 pt-1 z-10 w-full text-center")
                        li(class="")
                            router-link(class="rounded-t bg-gray-200 hover:bg-gray-400 py-2 px-12 block whitespace-no-wrap text-xs" to="home") Inicio
                        ...
                        div(class="dropdown inline-block relative")
                            ...
                            template(v-if="openMenu")
                                div(class="dropdown-menu absolute hidden text-gray-700 pt-1")
                                    ul( class="")
                                        li(class="")
                                            router-link(class="rounded-t bg-gray-200 hover:bg-gray-400 py-2 px-12 block whitespace-no-wrap text-xs" to="/") Inicio
                                        li(v-if="user_connected" class="")
                                            router-link(class="rounded-t bg-gray-200 hover:bg-gray-400 py-2 px-12 block whitespace-no-wrap text-xs" to="perfil") Mi Perfil
                                        li(v-if="user_connected" class="")                         
                                            a(class="rounded-t bg-gray-200 hover:bg-gray-400 py-2 px-12 block whitespace-no-wrap text-xs cursor-pointer" @click.prevent="logout()") Cerrar Sesión
